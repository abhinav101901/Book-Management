...
const createReview = async (req, res) => {
  try {
    ...
    return res.status(201).send({ status: true, message: "success", data: bookWithReviews })
...
const updateReview = async (req, res) => {
  try {
    ...
    if (review != undefined && typeof review !== "string") return res.status(400).send({ status: false, message: "Please enter valid review" });
