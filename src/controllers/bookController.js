...
const createBook = async (req, res) => {
  try {
    ...
    if (!userId || userId == "") return res.status(400).send({ status: false, message: "Please enter book userId" });
    if (!mongoose.isValidObjectId(userId)) return res.status(400).send({ status: false, message: "please enter valid userID" });
   

    if (ISBN != undefined && typeof (ISBN) != "string") return res.status(400).send({ status: false, message: "plese enter valid ISBN" });
    ...
    if(releasedAt){
      if(!checkDate(releasedAt))return res.status(400).send({status:false,message:"please enter valid format:YYYY-MM-DD"})
